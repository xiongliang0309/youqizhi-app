...
export const BubbleQuiz: React.FC<BubbleQuizProps> = ({ category, onExit }) => {
  ...
  return (
    <div className={`h-full flex flex-col font-sans relative overflow-hidden min-h-0 ${activeCategoryConfig?.bg || 'bg-blue-50'}`}>
      {/* 沉浸式背景光晕 */}
      ...
      {/* 顶部控制栏 */}
      <div className="p-4 md:p-6 flex items-center justify-between relative z-50">
        <div className="flex items-center">
          <button 
            onClick={onExit}
            className="bg-white p-3 md:p-4 rounded-full shadow-clay-card hover:scale-110 transition-transform border-[3px] border-white"
          >
            <ArrowLeft className="w-6 h-6 stroke-[3] text-gray-600" />
          </button>
          <div className="ml-4 bg-white/90 backdrop-blur px-5 py-2.5 rounded-full border-[3px] border-white shadow-clay-card">
        ...
        <div className="bg-white px-5 py-2.5 flex items-center rounded-full border-[3px] border-white shadow-clay-card">
      ...
      {/* 沉浸式泡泡答题区 */}
      <div className="flex-1 relative z-10 w-full flex flex-col items-center justify-center pb-12">
        <AnimatePresence mode="wait">
          ...
          <motion.div
            ...
            className="relative z-20 w-[90vw] md:w-[36rem] aspect-[4/3] md:aspect-square max-h-[40vh] flex flex-col items-center justify-center p-8 mb-8 md:mb-12
          ...
          >
            ...
            {/* 顶部星星装饰 */}
            <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 bg-yellow-400 px-6 py-2 rounded-full shadow-clay-card border-[3px] border-white flex items-center gap-1 z-30">
                <Star className="text-white fill-white w-5 h-5" />
                <Star className="text-white fill-white w-6 h-6" />
                <Star className="text-white fill-white w-5 h-5" />
            </div>

            <h2 className="text-4xl md:text-6xl lg:text-7xl font-black text-slate-800 text-center relative z-10 drop-shadow-sm leading-relaxed whitespace-pre-wrap">
              {level.question}
            </h2>
          </motion.div>
        </AnimatePresence>
        ...
        {/* 环绕选项泡泡群 */}
        <div className="relative z-30 w-full max-w-3xl flex justify-center items-center gap-6 md:gap-12 flex-wrap px-4">
          <AnimatePresence>
            {level.options.map((opt, idx) => {
              ...
              return (
                <motion.button
                  ...
                  animate={{ 
                    opacity: 1, 
                    y: isSelected ? 0 : [0, idx % 2 === 0 ? -10 : 10, 0], // 交错上下浮动
                    scale: isSelected && isCorrectOpt && feedback === 'correct' ? 1.3 : 1 
                  }}
                  transition={{ 
                  ...
                  className={`relative w-28 h-28 md:w-36 md:h-36 rounded-full flex items-center justify-center group outline-none
                             ...
                             ${isSelected && isCorrectOpt && feedback === 'correct' 
                                ? 'shadow-[0_0_40px_rgba(74,222,128,0.8),inset_6px_6px_12px_rgba(255,255,255,0.9),inset_-6px_-6px_15px_rgba(0,0,0,0.1)] ring-8 ring-green-400/50' 
                ...
                >
                  ...
                  <span className={`text-4xl md:text-5xl font-black drop-shadow-sm transition-colors z-10
          ...
          <AnimatePresence>
            {feedback !== 'none' && (
              <motion.div
                ...
                className={`px-8 py-4 rounded-full flex items-center text-xl md:text-2xl font-black text-white shadow-clay-card border-[3px] border-white ${
                  feedback === 'correct' ? 'bg-green-400' : 'bg-red-400'
                }`}
              >
                {feedback === 'correct' ? (
                  <><Check className="w-6 h-6 stroke-[4] mr-2" /> 答对啦！🎉</>
                ) : (
                  <><X className="w-6 h-6 stroke-[4] mr-2" /> 再试一次！💪</>
                )}
              </motion.div>
            )}
          </AnimatePresence>
